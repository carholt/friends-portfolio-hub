...
import {
  buildPreviewRows,
  buildPriceSymbol,
  computeStableHashFromNormalizedFields,
  detectBrokerByHeaders,
  mapNordeaExchange,
  parseCsvRows,
...
describe("Nordea transaction import parsing", () => {
  const fixture = readFileSync(
    resolve(process.cwd(), "src/test/fixtures/nordea-transactions.csv"),
    "utf8"
  );
  ...
  it("parses nordea fixture and extracts symbols", () => {
    ...
    expect(preview.every((row) => row.tx.price_symbol === "AYA:TSX")).toBe(
      true
    );
  ...
  it("maps TSX/TSXV and canonical price symbols", () => {
    expect(mapNordeaExchange("Toronto Stock Exchange")).toEqual({
      exchange_code: "TSX",
      price_symbol: "TMP:TSX",
    });

    expect(mapNordeaExchange("Toronto Venture Exchange")).toEqual({
      exchange_code: "TSXV",
      price_symbol: "TMP:TSXV",
    });
  ...
  it("re-imports missing trade_id rows without duplication using stable hash", () => {
    ...
    const upsertWithoutTradeId = (
      preview: ReturnType<typeof buildPreviewRows>
    ) => {
      let inserts = 0;
      let updates = 0;
  ...
  it("uses duplicate key precedence: trade_id first, stable_hash fallback", () => {
    ...
    expect(previewByTradeId[1].errors).toContain(
    ...
    expect(previewByStableHash[1].duplicateKey).toBe(
      previewByStableHash[0].duplicateKey
    );
    expect(previewByStableHash[1].errors).toContain(
      "Duplicate transaction in file"
    );
  });
});
