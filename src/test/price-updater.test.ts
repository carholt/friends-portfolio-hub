...
describe("update-prices edge function", () => {
  ...
  it("filters by stale price timestamps older than 10 minutes", () => {
    expect(source).toContain("STALE_AFTER_MS = 10 * 60 * 1000");
    expect(source).toContain("(nowMs - new Date(latest).getTime()) > STALE_AFTER_MS");
  });

  it("upserts prices by asset and price date then refreshes valuations", () => {
    expect(source).toContain("onConflict: \"asset_id,price_date\"");
