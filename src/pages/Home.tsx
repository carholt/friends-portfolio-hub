...
export default function HomePage() {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      const [{ data: values }, { data: profile }, { data: portfolios }] = await Promise.all([
        supabase.from("portfolio_valuations").select("portfolio_id,total_value,as_of_date").order("as_of_date", { ascending: false }).limit(500),
        supabase.from("profiles").select("onboarding_completed").eq("user_id", user!.id).maybeSingle(),
        supabase.from("portfolios").select("id").order("created_at", { ascending: false }).limit(1),
      ]);
      return { values: values || [], onboardingCompleted: profile?.onboarding_completed ?? false, hasPortfolio: (portfolios?.length || 0) > 0 };
    },
    enabled: !!user,
  });
  ...
  const completeOnboarding = async () => {
    if (!user) return;
    const { data: created } = await supabase.from("portfolios").insert({ name: portfolioName, base_currency: currency, owner_user_id: user.id, visibility }).select("id").single();
    if (created?.id) await logAuditAction("portfolio_create", "portfolio", created.id, { source: "onboarding" });
    if (created?.id && symbol && quantity) {
  ...
  const shouldShowOnboarding = !!data && !data.onboardingCompleted && !data.hasPortfolio;
    ...
    <AppLayout>
      ...
      <Dialog open={shouldShowOnboarding}>
        <DialogContent>
          ...
          {step === 3 && <div className="space-y-2"><Label>Choose visibility</Label><Select value={visibility} onValueChange={setVisibility}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="private">Private</SelectItem><SelectItem value="authenticated">Logged-in</SelectItem><SelectItem value="group">Group</SelectItem><SelectItem value="public">Public</SelectItem></SelectContent></Select><Button className="w-full" onClick={completeOnboarding}>Finish setup</Button></div>}
        </DialogContent>
      </Dialog>
